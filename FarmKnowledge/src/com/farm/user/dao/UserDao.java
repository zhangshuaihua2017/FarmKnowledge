...
public class UserDao {
	...
	public void addWaterAndFertilizer(int id, int water, int fertilizer) {
		...
		if(list.size()!=0) {
			User user = list.get(0);
			user.set("water", user.getInt("water")+water).update();
			user.set("fertilizer", user.getInt("fertilizer")+fertilizer).update();
		}
	}
	...
	public void addExandMoney(int id,int ex,int money) {
		...
		if(list.size()!=0) {
			User user = list.get(0);
			user.set("experience", user.getInt("experience")+ex).set("money", user.getInt("money")+money).update();
		}
	}
	//User表减少浇水（water）
	public void lessWater(int id) {
		...
		if(list.size()!=0) {
			User user = list.get(0);
			user.set("water", user.getInt("water")-1).update();
		}
	}
	//User表减少施肥（fertilizer）
		public void lessFertilizer(int id) {
			...
			if(list.size()!=0){
				User user = list.get(0);
				user.set("fertilizer", user.getInt("fertilizer")-1).update();
			}
		}
