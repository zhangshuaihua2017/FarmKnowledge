	...
	public void waterCrop(int userId,int ucId) {
		...
		if(list.size()!=0) {
			...
			if(uc.getInt("progress")>=100) {
				...
				UserCrop.dao.set("progress", uc.getInt("progress")+5).update();
			}else {
				UserCrop.dao.set("progress",100).update();
			}
		}
	...
	public void fertilizerCrop(int userId,int ucId) {
		...
		if(list.size()!=0) {
			...
			if(uc.getInt("progress")>=100) {
				...
				UserCrop.dao.set("progress", uc.getInt("progress")+10).update();
			}else {
				UserCrop.dao.set("progress", 100).update();
			}
		}
		...
		if(list.size()!=0) {
			...
			if(uc.getInt("progress")==100) {
				if(uc.getInt("status")==0) {
					...
					UserCrop.dao.set("status", 1).update();
				}
			}
		}
	}
	//种植新作物,,,(没有考虑当前土地是否已有作物占用，由前端考虑QAQ）
	public void addNewCrop(int userId,String landNumber,int cropId) {
